...
sir_plot <- function(sir_data, outcome, pop_level) {
  ...
  if (pop_level == "states") {
    if (outcome == 1){
      return(
        plotly::ggplotly(ggplot2::ggplot(data = sir_data, ggplot2::aes(y = (sir - 1), x = reorder(state.x, sir), fill = typeC)) + 
                           ggplot2::geom_bar(stat = "identity", position = dodge) + 
                           ggplot2::geom_errorbar(ggplot2::aes(ymax = (usir - 1), ymin = (lsir - 1 )), position = dodge, width = 0.25) + 
                           ggplot2::xlab("State") + 
    ...
    if (outcome == 0) {
      return(
        plotly::ggplotly(ggplot2::ggplot(data = sir_data, ggplot2::aes(y = (sdr - 1), x = reorder(state.x, sdr), fill = typeD)) + 
                           ggplot2::geom_bar(stat = "identity", position = dodge) + 
    ...
    if (pop_level == "counties") {
      ...
      if (outcome == 1) {
        return(
          plotly::ggplotly(ggplot2::ggplot(data = sir_data, ggplot2::aes(y = (sir - 1), x = reorder(county.x, sir), fill = typeC)) + 
                             ggplot2::geom_bar(stat = "identity", position = dodge) + 
                             ggplot2::geom_errorbar(ggplot2::aes(ymax = (usir - 1), ymin = (lsir - 1 )), position = dodge, width = 0.25) + 
